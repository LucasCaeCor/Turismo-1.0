...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['imagem']) && $_FILES['imagem']['error'] == 0) {
        $targetFile = $diretorioUploads . basename($_FILES['imagem']['name']);
        ...
        if (in_array($imageFileType, $allowedTypes)) {
            ...
            if (move_uploaded_file($_FILES['imagem']['tmp_name'], $targetFile)) {
                ...
                if ($stmt->execute()) {
                    $resposta = "<p class='text-success'>Solicitação enviada com sucesso!</p>";
                } else {
                    $resposta = "<p class='text-danger'>Erro ao enviar a solicitação: " . $conn->error . "</p>";
                }
                $stmt->close();
            } else {
                $resposta = "<p class='text-danger'>Erro ao mover o arquivo para o diretório de uploads.</p>";
            }
        } else {
            $resposta = "<p class='text-danger'>Tipo de arquivo não permitido. Apenas JPG, JPEG, PNG e GIF são aceitos.</p>";
        }
    } else {
        $resposta = "<p class='text-danger'>Erro no envio da imagem.</p>";
    }
}
?>
